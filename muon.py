...
def muon_update(grad, momentum, beta=0.95, ns_steps=5, nesterov=True):
    """Compute the Muon update for a parameter."""
...
def adam_update(grad, buf1, buf2, step, betas, eps):
    """Compute the AdamW update for a parameter."""
...
class MuonWithAuxAdam(torch.optim.Optimizer):
    """
    Muon variant that can be used for all parameters in the network, since it runs an
    ...
    Example usage:
        ...
        other_params += [*model.head.parameters(), *model.embed.parameters()]
        param_groups = [
